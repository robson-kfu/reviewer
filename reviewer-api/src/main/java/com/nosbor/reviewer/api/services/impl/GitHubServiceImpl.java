...
public class GitHubServiceImpl implements IVSCService {
    ...
    public GitHubServiceImpl(@Value("${" + VCS_SERVICES_GITHUB_INSTALLATION_ID + ":}") String installationId, @Value("${" + VCS_SERVICES_GITHUB_APP_ID + ":}") String appId, @Value("${" + VCS_SERVICES_GITHUB_PEM_PATH + ":}") String pemPath, @Value("${" + VCS_SERVICES_GITHUB_BASE_URL + ":}") String baseUrl, @Value("${" + VCS_SERVICES_GITHUB_API_BASE_URL + ":}") String apiBaseUrl, @Value("${" + VCS_SERVICES_GITHUB_API_VERSION + ":}") String apiVersion) {
        ...
        this.baseClient = WebClient.builder().baseUrl(baseUrl).defaultHeader("Accept", "text/html").build();
        this.apiClient = WebClient.builder().baseUrl(apiBaseUrl).defaultHeader("Accept", "application/vnd.github+json")
                .defaultHeader("X-GitHub-Api-Version", apiVersion).build();
    }
        ...
        apiClient
                ...
                .uri(uriBuilder ->
                        uriBuilder
                                ...
                                .build(aiResponseWrapper.getOwner(),
                                        aiResponseWrapper.getRepoName(),
                                        aiResponseWrapper.getPullRequestId()))
    ...
    private String commentBody(List<CommentTO> commentTOList) {
        return """
                {
                     "body": "Revise os comentários realizados pela IA",
                     "event": "COMMENT",
                     "comments": [
                         %s
                     ]
